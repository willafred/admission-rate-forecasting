...
In this file, we begin analysis of the raw admissions of COPD and asthma cases. 
...
new_combined_copd = data_editing_subgroups("combined_asthma_env_080225_norates.csv", outcome = "male_copd", significant_predictors = copd_significant_predictors)
new_combined_asthma = data_editing_subgroups("combined_asthma_env_080225_norates.csv", outcome = "age_85_above_asthma", significant_predictors = asthma_significant_predictors)
...
The function for this chunk describes the overall function for the h-week ahead forecasting of the raw admission cases for Asthma and COPD cases for a given week.
...
# stopCluster()
regression_model_fn_rawadmissions = function(df_link, outcome_var, copd_lag_num = 1, asthma_lag = 1, env_lag = 1, horizon, training_limit=0.5, significant_predictors){
  ...
  results = foreach(i = (training_length):(n-1), .combine = rbind, .packages = c("dplyr", "mgcv","stringr","gamlss","gamsel","MASS","xgboost","h2o")) %dopar% {
    ...
    pure_ar_formula_str = paste0(outcome_var, " ~ offset(log(", lagged_disease_popn, ")) + ", paste(paste0(pure_ar_lagged_terms, ")"), collapse = " + "))
    
    pure_ar_model = bam(as.formula(pure_ar_formula_str), family = nb(link = "log", theta = initial_theta), data = train_standardised)

    pure_ar_predictions = predict(pure_ar_model, newdata = test_standardised, type = "response")
    
    print("Pure AR Done")
    
    # Model 2: AR (Other Disease)
    ...
    ar_mixed_model = bam(as.formula(ar_mixed_formula_str), family = nb(link = "log", theta = initial_theta), data = train_standardised)
    ...
    glm_variables_to_lag = paste0("pb(",outcome_var)
    glm_lagged_terms = unlist(lapply(glm_variables_to_lag, function(var) {
      paste0(var, "_L", 1:copd_lag_num)
    }))
    base_terms = paste0(outcome_var, " ~ offset(log(", lagged_disease_popn, ")) + ", paste(paste0(glm_lagged_terms, ")"), collapse = " + "))
    ...
    gam_lss_predictions = tryCatch({
      ...
      gam_lss_obj = gamlss(overall_formula, family = NBI(mu.link = "log"), data = train_standardised)
    ...
    pure_ar_formula_str_ns = paste0(outcome_var, " ~ offset(log(", lagged_disease_popn, ")) + ", paste(pure_ar_lagged_terms_ns, collapse = " + "))
    ...
    glm_model = glm(as.formula(ar_mixed_formula_str_ns), family = negative.binomial(theta=initial_theta), data = train_standardised) 
    ...
    gamsel_training = train_standardised %>% dplyr::select(-c(outcome_var,lagged_disease_popn))
    ...
    train_matrix = as.matrix(train_standardised %>% dplyr::select(-c(outcome_var,lagged_disease_popn)))
    ...
    testing_set_gamsel = test_standardised %>% dplyr::select(-c(lagged_disease_popn))
    ...
    absolute_err = abs(glm_gamsel_predictions - test_set[[outcome_var]])
    error = mean((abs(glm_gamsel_predictions - test_set[[outcome_var]])) / test_set[[outcome_var]], na.rm = TRUE)
    ...
    gam_model = bam(as.formula(ar_mixed_formula_str), family = nb(link = "log", theta = initial_theta), data = train_standardised)
    ...
    # Generation of combined prediction
    ...
    train_standardised_automl = train_standardised %>%
      mutate(!!outcome_var := log(!!sym(outcome_var)),
             !!lagged_disease_popn := log(!!sym(lagged_disease_popn)))
    ...
    aml_prediction = exp(as.data.frame(h2o.predict(aml, newdata = aml_test_set))[1,])
    actual_value = exp(as.data.frame(aml_test_set[[response]])[1,])
    
    result_row = data.frame(
      actual = test_set[[outcome_var]],
      pure_ar_predictions = pure_ar_predictions,
      ar_other_disease_predictions = ar_other_disease_predictions,
      ar_env_predictions = ar_env_predictions,
...
Finally, running of the code then commences for the RAW admissions for COPD and Asthma in hospitals.
```{r}
all_asthma_cases_factors = c("cases")
all_copd_cases_factors = c("cases")

for (asthma_factor in all_asthma_cases_factors){
  for (horizon in 1:12){
    full_asthma_factor = paste0(asthma_factor,"_asthma")
    print(paste("Starting", full_asthma_factor, "horizon",horizon))
    submodels_asthma = regression_model_fn_rawadmissions(df_link = "combined_asthma_env_080225_norates.csv",outcome_var = full_asthma_factor,horizon=horizon,significant_predictors = asthma_significant_predictors)
...
for (copd_factor in all_copd_cases_factors){
  for (horizon in 1:12){
    full_copd_factor = paste0(copd_factor,"_copd")
    print(paste("Starting", full_copd_factor,"horizon",horizon))
    submodels_copd = regression_model_fn_rawadmissions(df_link = "combined_asthma_env_080225_norates.csv",outcome_var = full_copd_factor,horizon=horizon, significant_predictors = copd_significant_predictors)
